import {
  TWITCH_USERS_FOLLOWS,
  TWITCH_GET_STREAMS,
  TWITCH_QUERY_CHANNELS,
  TWITCH_GAMES,
  TWITCH_USERS_URL,
} from './consts'
...
export const buildFollowsUrl = ({ after, to_id, from_id, first = 100 }) => {
...
export const buildStreamsUrl = ({ user_id, user_login, game_id, first, before, after }) => {
  ...
  return `${TWITCH_GET_STREAMS}?${user_id ? `user_id=${user_id}` : ''}${first ? `&first=${first}` : ''}${
    after ? `&after=${after}` : ''
  }`
}
...
export const buildStreamsQueryUrl = ({ query, first, after: cursor, liveOnly = false }) => {
  ...
  return `${TWITCH_QUERY_CHANNELS}?${query ? `query=${query}` : ''}${first ? `&first=${first}` : ''}${
    cursor ? `&after=${cursor}` : ''
  }&live_only=${liveOnly}`
}
